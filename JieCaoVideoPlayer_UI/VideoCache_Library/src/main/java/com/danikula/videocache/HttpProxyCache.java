...
import com.danikula.videocache.optimize.OkHttpUrlSource;
...
class HttpProxyCache extends ProxyCache {
    ...
    private void responseWithoutCache(OutputStream out, long offset) throws ProxyCacheException, IOException {
       // HttpUrlSource newSourceNoCache = new HttpUrlSource(this.source);
        OkHttpUrlSource okHttpUrlSource = new OkHttpUrlSource(this.source.getUrl());
        try {
            //newSourceNoCache.open((int) offset);
            okHttpUrlSource.open((int) offset);
            byte[] buffer = new byte[DEFAULT_BUFFER_SIZE];
            int readBytes;
            while ((readBytes = okHttpUrlSource.read(buffer)) != -1) {
                out.write(buffer, 0, readBytes);
                offset += readBytes;
            }
            out.flush();
